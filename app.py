...
def analyze_sentiment_and_users(comments_data):
    ...
    stopwords = set([
        "the", "and", "is", "it", "to", "in", "this",
        "of", "a", "for", "on", "my", "that", "with",
        "video", "are", "but", "so", "be", "was", "i",
        "you", "me", "have", "just", "like", "your", "get"
    ])
    ...
    for item in comments_data:
        ...
        clean_text = ''.join([
            ch.lower() if ch not in punctuation else ' '
            for ch in text
        ])
...
def build_keyword_frequency_and_sentiment(
    comments_with_scores,
    title_and_desc,
    top_n=12
):
    ...
    if not candidates:
        ...
        candidates = [
            w for w, _
            in comment_word_counter.most_common(20)
        ]

    freq_map = Counter()

    sentiment_map = defaultdict(
        lambda: {
            "pos": 0,
            "neu": 0,
            "neg": 0,
...
def analyze_video():
    ...
    try:
        ...
        analysis = analyze_sentiment_and_users(
            comments
        )

        comments_with_scores = analysis.get(
            "comments_with_scores",
            []
        )
        ...
        keyword_freq, keyword_sentiment = (
            build_keyword_frequency_and_sentiment(
                comments_with_scores,
                title_desc,
                top_n=12
            )
        ...
        response = {
            ...
            "analysis": {
                ...
                "top_positive_comments":
                    analysis["top_positive_comments"],

                "top_negative_comments":
                    analysis["top_negative_comments"],

                "common_topics":
                    analysis["common_topics"],

                "toxic_users":
                    analysis["toxic_users"]
            },
